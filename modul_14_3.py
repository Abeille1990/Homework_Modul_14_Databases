...
async def get_buying_list(message):
    with open('img/1.jpg', 'rb') as img:
        await message.answer_photo(img,
                                   f'Название: Welllab UNCARIA FORTE | Описание: БАД для'
                                   f' поддержки иммунитета | Цена: 1 190')
    with open('img/07034-1.jpg', 'rb') as img:
        await message.answer_photo(img,
                                   f'Название:  Welllab ANTISWEET CONTROL | Описание: БАД для'
                                   f' поддержки углеводного обмена | Цена: 1 350')
    with open('img/07035-1.jpg', 'rb') as img:
        await message.answer_photo(img,
                                   f'Название: Welllab BRONCHOLUX INTENSIVE | Описание: БАД для'
                                   f' дыхательной системы | Цена: 1 350')
    with open('img/Welllab_vision_7058_product_box.jpg', 'rb') as img:
        await message.answer_photo(img,
                                   f'Название: Welllab VISION COMPLEX | Описание: БАД для'
                                   f' поддержки зрения | Цена: 1 500')
